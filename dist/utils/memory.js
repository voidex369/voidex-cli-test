...
export function truncateForRAM(content) {
    if (!content)
        return '';
...
export function pruneHistoryByChars(messages) {
    let totalChars = 0;
    const result = [];
    for (let i = messages.length - 1; i >= 0; i--) {
        const m = messages[i];
        const mChars = (m.content?.length ?? 0) +
 ...
 */
export function trimMessagesToTokenLimit(messages, tokenLimit) {
