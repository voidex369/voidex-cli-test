...
export async function runShellCommand({ command, onOutput, timeout }) {
    ...
    return new Promise((resolve) => {
        ...
        const isInteractivePrompt = (text) => {
            const lines = text.trim().split('\n');
            const lastLine = lines[lines.length - 1] || '';
            const cleanLine = lastLine.trim().toLowerCase();
            // 1. Cek pola spesifik (High Confidence)
            if (/(\[y\/n\]|\(y\/n\)|\? y\/n|password:|passphrase:|confirm\s*:)/i.test(cleanLine))
                return true;
            ...
            if (endsWithSymbol && cleanLine.length < 80)
                return true;
            // 3. Cek kata kerja perintah input
            const hasInputKeywords = /^(enter|select|choose|type|provide|input)\s+/i.test(cleanLine);
            if (hasInputKeywords && !cleanLine.includes('...'))
                return true; // Hindari "Enter ... to continue" yang cuma info
            return false;
        };
        const handleProcessActivity = (chunk) => {
...
// 5. Glob
export async function glob({ pattern, path: searchPath = '.' }) {
    try {
        const results = [];
...
// 6. SearchText
export async function searchText({ query, path: searchPath = '.' }) {
    try {
        const results = [];
        const baseDir = path.resolve(process.cwd(), searchPath);
        async function walk(dir) {
            ...
            for (const entry of entries) {
                ...
                if (entry.isDirectory()) {
                    ...
                    try {
                        ...
                        const matches = lines.map((line, idx) => line.includes(query) ? `${relPath}:${idx + 1}: ${line.trim()}` : null).filter(Boolean);
                        if (matches.length > 0)
...
export async function webFetch({ url }) {
    try {
        ...
        if (/image|video|audio|pdf|zip/.test(contentType))
            return { output: `[BINARY: ${contentType}]`, isError: false };
...
// 8. Replace
export async function replace({ path: filePath, oldText, newText }) {
    try {
        const absPath = path.resolve(process.cwd(), filePath);
...
// 9. SaveMemory
export async function saveMemory({ info }) {
    try {
