...
export class LocalExecutor {
    ...
    analyzeRisk(toolName, args) {
        ...
        if (toolName === 'run_shell_command' || toolName === 'execute_bash') {
            ...
            if (/(^|\s)rm\s+(-[a-zA-Z]*r[a-zA-Z]*|-[a-zA-Z]*f[a-zA-Z]*)\s+/.test(cmd))
                return { level: 'critical', reason: 'Force Delete (rm -rf)' };
            if (/(^|\s)mkfs/.test(cmd))
                return { level: 'critical', reason: 'Format Disk' };
            if (/(^|\s)dd\s+/.test(cmd))
                return { level: 'critical', reason: 'Low-level Disk Write' };
            if (/(^|\s):(){:|:&};:/.test(cmd))
                return { level: 'critical', reason: 'Fork Bomb Detected' };
            if (/(^|\s)(shutdown|reboot|init\s+0|init\s+6)/.test(cmd))
                return { level: 'critical', reason: 'System Power Control' };
            // --- B. CAUTION (MODERATE TIER) - Butuh Y/N ---
            // Root access & System modification.
            if (/(^|\s)sudo\s+/.test(cmd))
                return { level: 'caution', reason: 'Root Access (sudo)' };
            if (/(^|\s)(chmod|chown)\s+/.test(cmd))
                return { level: 'caution', reason: 'Permission Modification' };
        ...
        while (state !== 'DONE' && state !== 'ERROR') {
            ...
            if (state === 'THINKING') {
                ...
                try {
                    ...
                    let assistantMsg = { id: 'assistant-' + Date.now(), role: 'assistant', content: '', tool_calls: [] };
                    let toolCallsBuffer = [];
                    let lastUpdate = 0;
                    ...
                    for await (const chunk of stream) {
                            ...
                            for (const tc of delta.tool_calls) {
                                if (tc.index !== undefined) {
                                    if (!toolCallsBuffer[tc.index])
                                        toolCallsBuffer[tc.index] = { id: tc.id, type: tc.type, function: { name: '', arguments: '' } };
                                    if (tc.id)
                        ...
                        // [PERBAIKAN SCROLL] Optimize batching untuk mencegah scroll thrashing
                        // Interval 200ms lebih stabil untuk chat panjang
                        if (hasChanges) {
                            if (!isAddedToHistory) {
                ...
                for (const tc of lastMsg.tool_calls) {
                    ...
                    try {
                        ...
                        const result = await toolFunc({ ...args, onOutput: (chunk) => onLiveOutput(chunk) });
                        onLiveOutput('');
                        // [FIX] Mengirim status error secara eksplisit melalui format string JSON
                        // Ini memastikan MessageItem.tsx bisa membaca status tanpa ragu
                        const payload = {
                            isError: result.isError,
                            output: result.output || 'Done'
                        };
                        // Konversi ke string JSON agar muat di Message.content
                        let content = JSON.stringify(payload);
                        content = truncateForRAM(content);
                        toolMessages.push({
                        ...
                        toolMessages.push({
                            ...
                            tool_call_id: tc.id,
                            name: toolName,
                            content: JSON.stringify({ isError: true, output: `[TOOL CRASH] ${err.message}` })
                        });
                    }
                }
...
4. Achieve the objective at all costs.`;
    ...
    detectLoop(msgs) {
        ...
        if (sigs[last] === sigs[last - 1] && sigs[last] === sigs[last - 2])
            return true;
        ...
        const allEmpty = recentAssistants.length >= 4 && recentAssistants.every(m => (!m.content || !m.content.trim()) && (!m.tool_calls || m.tool_calls.length === 0));
        if (allEmpty)
            return true;
        return false;
    }
