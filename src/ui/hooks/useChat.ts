...
// --- INTERACTIVE SESSION SELECTION STATE (FILE SCOPE) ---
    ...
    const sendMessage = useCallback(async (rawContent: string) => {
        ...
        const sessionModeActive = (window as any).__sessionMode;
        if (sessionModeActive && (content === '' || content.toLowerCase() === 'cancel')) {
            delete (window as any).__sessionMode;
            delete (window as any).__sessionList;
            setAgentStatus(null);
            ...
            if (sub === 'resume') {
                if (arg) {
                    ...
                    if (list.length === 0) {
                        ...
                        (window as any).__sessionList = list;
                        (window as any).__sessionMode = 'resume';
            ...
            if (sub === 'delete') {
                if (arg) {
                    ...
                    if (list.length === 0) {
                        ...
                        // SIMPAN LIST UNTUK UI SELECTION
                        (window as any).__sessionList = list;
                        (window as any).__sessionMode = 'delete';
        ...
        if (lowerContent === '/debug') {
            ...
            recentLogs.reverse().forEach((log, index) => {
                ...
                if (log.statusCode) debugContent += `• Status: ${log.statusCode}\n`;
                debugContent += `• Message: ${log.errorMessage}\n`;
                if (log.suggestion) debugContent += `• Suggestion: ${log.suggestion}\n`;
                debugContent += `\n`;
            });
    ...
    // Handle angka selection dari user input
        // Handle angka selection dari user input
    const handleSessionSelection = useCallback((input: string): boolean => {

        const selectionMode = (window as any).__sessionMode;
        const sessionList = (window as any).__sessionList;
        
        if (!selectionMode || !sessionList) return false;
        ...
        if (index >= 0 && index < sessionList.length) {
            ...
            if (selectionMode === 'resume') {
                ...
                if (success) {
                    ...
                    (window as any).__sessionList = updated;
                    if (updated.length === 0) {
                        (window as any).__sessionList = [];
                        (window as any).__sessionMode = null;
                    }
                }
            }
